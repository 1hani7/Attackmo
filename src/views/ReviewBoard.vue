        ...
        <div id="wrap">
            ...
            <div class="main">
                ...
                <table class="list">
                    ...
                    <tbody>
                        <tr class="con" v-for="(item, index) in displayedReview" :key="index">
                ...
                <div class="mobileList">
                    <div class="post" v-for="(item, index) in displayedReview" :key="index">
...
export default {
    setup(){
        ...
        const pageCount = computed(() => Math.ceil(list.length / itemsPerPage.value));

        const filteredReviews = (event) => {
            console.log(event.target.value)
            // const word = searchQuery.value.trim().toLowerCase();
            const word = event.target.value;
            const title = document.querySelectorAll(".t");
            title.forEach(function(item){
                item.parentNode.classList.remove('hide');
                if(item.innerText.indexOf(word) == -1){
                    item.parentNode.classList.toggle('hide');
                }
            })
            // return list.filter(review => review.rTitle.toLowerCase().includes(word));
        };
        ...
        // 이제 라우터를 조작하려면 여기에서 라우터를 사용하실 수 있습니다.
        }

        // 검색어가 변경되면 페이지를 1로 재설정합니다.
        watch(searchQuery, () => {
        currentPage.value = 1;
        });
